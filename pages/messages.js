...
function Messages({ chatsData, user }) {
  ...
  useEffect(() => {
    ...
    if (socket.current) {
      ...
      // render the first chat if there exists chats
  ...
  useEffect(() => {
    if (socket.current) {
      // detect msgSent event where receiver is current user
      ...
      socket.current.on("newMsgReceived", async ({ newMsg }) => {
        ...
        if (newMsg.sender === openChatId.current) {
          // when the chat is currently opened inside browser
          setMessages((prev) => [...prev, newMsg]);
          ...
          if (ifPreviouslyMessaged) {
            ...
            // when the sender has no previous chat
